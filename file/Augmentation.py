...
def _ensure_same_shape(orig, processed: np.ndarray):
    """
    Resize and clip the processed image to match the original
    shape and uint8 type.
    """
    if processed.shape != orig.shape:
        ret = ski.transform.resize(
            processed, orig.shape, anti_aliasing=True, preserve_range=False)
...
def rotate(image, angle):
    ...
    rotated = ski.transform.rotate(
        image, angle, resize=True, mode='constant', cval=1)
...
def contrast(image, gain):
    ...
    adjusted = ski.exposure.adjust_gamma(
        image, gain)  # type: ignore -- it's a float input!
...
def shear(image, shear_angle):
    """
    Applies a perspective/shear transform to the image along the x-axis
    while keeping the entire image in view.
    """
...
def process_one_image(image_path: Path, out_dir):
    ...
    augmented = [
        ...
        # type: ignore - gamma <1 => brighter
        ("contrast", contrast(img, contrast_factor)),
