...
def evaluate_model(restored_keras_model: Any, ds_test: Any, predict_dataset: Any, class_names: list[str]):

    # replace remote tfds load with loading from local directories
    # test_loss, test_acc = restored_keras_model.evaluate(ds_test)
    # print(f"Model loaded from {keras_model_path} — test_loss={test_loss:.4f}, test_acc={test_acc:.4f}")
    ...
    # print per-class summary: total files, correct (true), incorrect (false), accuracy %
    num_classes = len(class_names)
    print("\nPer-class summary:")
    for idx in range(num_classes):
        total = y_true.count(idx)
        correct = sum(1 for t, p in zip(y_true, y_pred) if t == idx and p == idx)
        wrong = total - correct
        accuracy = (100.0 * correct / total) if total > 0 else 0.0
        print(f"{class_names[idx]}: total={total}, true={correct}, false={wrong}, accuracy={accuracy:.1f}%")
